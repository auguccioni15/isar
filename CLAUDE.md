...
- The kernel recipe (`meta-isar/recipes-kernel/linux/linux-stm32mp_6.1.bb`) patches `dwc3-stm32.c` to add a missing `bitfield.h` include and injects `stm32mp157c-dk2-wifi.dtsi` into `stm32mp157c-dk2.dts` to describe the SDIO WiFi node.
...
- **DRM card ordering**: The Etnaviv GPU (GC400, present on all MP157 variants) probes first and claims `card0`; the LTDC display controller claims `card1`. `weston.service` passes `--drm-device=card1 --use-pixman` and waits for `/dev/dri/card1` via a poll loop (`until [ -e /dev/dri/card1 ]`) rather than `Requires=dev-dri-card1.device` — the systemd device unit can time out before LTDC completes its deferred probe. `--use-pixman` is required because LTDC is a display-only controller with no GPU — Weston's EGL/GBM backend fails on it; pixman does CPU rendering and hands the framebuffer to LTDC. `DRM_STM` (LTDC) and `DRM_ETNAVIV` (GPU) are both enabled.
